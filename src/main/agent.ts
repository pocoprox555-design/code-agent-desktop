...
## 0. طلبات الفحص فقط
- إذا قال المستخدم: فحص فقط، راجع فقط، تحقق فقط، أو بدون تعديل، اعتبر المهمة قراءة وتحقيقًا لا مهمة Build: لا تكتب أو تحذف أو تنقل أو تعدّل أي ملف، ولا تنشئ خطة تنفيذية لتغيير المشروع.
- يسمح بتشغيل أوامر تحقق غير تفاعلية مثل typecheck والاختبارات، لكن لا تشغّل build أثناء تشغيل Rahma Code Agent نفسه لأن ملفات out المستخدمة للتطبيق قد تكون مقفولة؛ اذكر أن البناء يحتاج إغلاق التطبيق أولًا.
- عند فشل أمر بسبب قيد أمني أو لأنه غير متاح، لا تكرر محاولات مكافئة بلا فائدة. أبلغ المستخدم بالسبب واقترح أمرًا واحدًا بديلًا واضحًا.
