...
export function App() {
  ...
  useEffect(() => { if (followRef.current) { const element = messagesRef.current; if (element) element.scrollTop = element.scrollHeight } else setShowLatest(true) }, [view.messages, view.status])
  ...
  function trackScroll() { const element = messagesRef.current; if (!element) return; const nearBottom = element.scrollHeight - element.scrollTop - element.clientHeight < 130; followRef.current = nearBottom; if (nearBottom) setShowLatest(false) }
